<template>
  <div class="recommend">
      <div class="list">
        <div class="title"><span>你的宝藏歌单库</span> <span @click="toList">更多<van-icon name="arrow" /></span>  </div>
        <div>
            <van-swipe :show-indicators='false' :loop='false' :width="110">
              <van-swipe-item  v-for="(item,index) of [1,2,3,4,5,6]" :key="index">
                <ul>
                 <li v-for=" (L,i) of reList.slice(index*2,item*2)" :key="L.content_id" @click="toListInfo(index,i)">
              <div>
                <img :src='L.cover' >
                <span>{{L.title}}</span>
              </div>
...
export default {
 ...
 methods:{
   ...
   toListInfo(index,i){
     this.$router.push(`/songlistinfo/${this.reList[index*2+i].content_id}`)

<template>
  <div class="club">
    ...
    <div class="content">
      ...
      <div class="new">
        <van-tabs v-model="active" background='#f5f5f5'
           color='rgb(22, 211, 32)'>
             <van-tab title="新歌">
                <van-swipe  class="ranklist" :show-indicators='false' :loop='false'>
            <van-swipe-item v-for="(item,index) of [1,2,3,4]" :key="index">
              <ul>
                <li v-for="(song,i) of newSongs.slice(index*3,item*3)" :key="song.id"
                 @click="play(index,i)" >
             ...
             <van-swipe-item v-for="(item,index) of [1,2]" :key="index">
              <ul>
                <li v-for="(album,i) of newAlbum.slice(index*3,item*3)" :key="album.id" @click="toAlbum(i,index,item)">
          ...
          <van-tab title="新MV">
            ...
            <van-swipe-item v-for="(item,index) of [1,2,3,4]" :key="index">
              <ul>
                <li v-for="(mv,i) of newMv.slice(index*3,item*3)" :key="mv.mv_id" @click="toMv(i,index,item)">
                  <div class="img" >
                    <img :src="mv.picurl" >
                  </div>
                  <div class="newsong">
      ...
      <div class="rank">
         <div><span>排行榜</span> <span @click="toRank">更多<van-icon name="arrow" /></span></div>
         <van-swipe  class="ranklist" :show-indicators='false' :loop='false'>
          <van-swipe-item v-for="(item,index) of nowRankList" :key="item.topId" @click="toRankInfo(index)">
            <div class="swp">
              <div class="img">
                <img :src='item.picUrl' >
              </div>
...
export default {
 data(){
    return{
      ...
      tab:[{title:'歌手',to:'/singer',icon:'iconfont icon-yonghu'},
      {title:'排行',to:'/rank',icon:'iconfont icon-paixing'},
      {title:'分类歌单',to:'/songList',icon:'iconfont icon-dilanxianxingiconyihuifu_huabanfuben'},
 ...
 methods:{
  ...
  toRankInfo(index){
    this.$router.push(`/topdetail/${this.rankList[0].list.slice(0,4)[index].topId}`)
  },
  toAlbum(i,index,item){
    this.$router.push(`/albuminfo/${this.newAlbum.slice(index*3,item*3)[i].mid}`)
  },
  toMv(i,index,item){
    this.$router.push(`/mvinfo/${this.newMv.slice(index*3,item*3)[i].vid}`)
  },
  getInfo(){

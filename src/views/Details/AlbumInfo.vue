    ...
    <div class="top">
      ...
      <div class="info">
         ...
         <div class="title">
           ...
           <div><div class="desc">简介：{{info.desc}}</div>
           <span @click="showInfo"><van-icon name="arrow" /></span></div>
       ...
       <div class="list">
            ...
            <ul>
                <li v-for="(item,index) of songs.list" :key="item.id" @click="play(index)">
                    <div class="song">
                        <span class="size">{{index+1}}</span>
                        <div class="name"> 
                            <span  class="size">{{item.title}}</span>
                         <div>
                            <span v-for="singer of item.singer" :key="singer.id">{{singer.name}} </span></div> 
                        </div>
                    </div>
                    <div> <span @click="playmv(index)"  class="iconfont icon-lianhe11 mv"></span></div>
                </li>
            </ul>
        </div>
  </div>
...
export default {
  ...
  methods:{
    ...
    async getInfo(){
      let res=await this.$http.get('/album?albummid='+this.$route.params.albummid)
      this.info=res.data.data
    },
    async getSong(){
      let res=await this.$http.get('/album/songs?albummid='+this.$route.params.albummid)
    ...
    async play(index){
      ...
      let res=await this.$http.get('/song/urls?id='+this.songs.list[index].mid)

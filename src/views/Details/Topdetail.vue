<template>
  <div class="topdetail">
      <div class="head">
        ...
        <span>{{topInfo.update}} 更新 <van-icon name="info-o" size="15" class="icon" @click="showInfo"/></span>
      </div>
      <div>全部播放 ({{topInfo.total}})</div>
      <div class="songs">
        <ul>
          <li v-for="(item,index) of topInfo.list" :key="item.songId" @click="play(index)">
            <div class="rank">
              <div>{{item.rank}}</div>
            ...
            <div class="song">
              <div>{{item.title}}</div>
              <div> <span>{{item.singerName}}</span>- <span>{{item.album.title}}</span> </div>
            </div>
          </li>
        </ul>
      </div>
...
export default {
  ...
  methods: {
    async fetchTopDetail(){
      let res= await this.$http.get('/top',{params:{id:this.$route.params.id}})
    ...
    async play(index){
      ...
      let res=await this.$http.get('/song/urls?id='+this.topInfo.list[index].mid)
